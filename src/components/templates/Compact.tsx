  ...
  return (
    <article className={sectionClassName} style={{ '--primary': primaryColor ?? options.accentColor } as CSSProperties}>
      <header className="resume-header resume-compact-header">
        ...
        <aside className="resume-compact-meta">
          {resume.basics.location ? <p>{resume.basics.location}</p> : null}
          {resume.basics.email ? <p>{resume.basics.email}</p> : null}
          {resume.basics.phone ? <p>{resume.basics.phone}</p> : null}
        </aside>
      </header>

      <div className="resume-compact-grid">
        <div className="resume-compact-column">
          {primarySections.map((sectionId) => renderSection(sectionId))}
        </div>
        <div className="resume-compact-column">
          {secondarySections.map((sectionId) => renderSection(sectionId))}
        </div>
      </div>
    </article>
  )
